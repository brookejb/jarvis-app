...
- If it's Sunday and the weekly review hasn't been done yet, mention it once naturally when planning comes up: "Weekly review isn't done yet - worth 5 minutes before the week starts." Don't push it, just name it.
- If it's Sunday evening and she hasn't mentioned the week ahead, offer: "Want me to run through the week with you?"
- If the weekly review was done this week and she has a week intention set, reference it when it's relevant - not every message, just when it genuinely connects.
...
export default async function handler(req, res) {
  ...
  const { messages, priorities, weekHabits, energy, todayDW, racingChecklist, semesterGoals, lifeGoals, sprintItems, routineLog, windDownLog, weeklyReviewLog, weekIntention, checkedPriorities, clientTime } = req.body;
  ...
  const MODE_CONTEXT = {
    student: `\n\nACTIVE MODE: Student. Brooke is in heads-down academic mode. Lead with classes, Canvas deadlines, problem sets, exam prep, and deep work blocks. Academic work is the primary focus. M Racing context is still in your system - if an M Racing deadline is within 24 hours or something is truly urgent, flag it briefly and move on. Otherwise stay academic.`,
    racing: `\n\nACTIVE MODE: M Racing. Brooke is in racing/team mode. Lead with M Racing tasks, team meetings, Wilson Center schedule, business subteam responsibilities, and Director role progress. Academic context is still in your system - if a Canvas deadline is within 48 hours, mention it once as a quick flag ("heads up, EECS HW due tomorrow") and move on. Don't build academic plans in this mode, just surface imminent pressure.`,
    builder: `\n\nACTIVE MODE: Builder. Brooke is thinking long-horizon. Lead with the Ross application, the team website, anything she's constructing toward her future. The Sydney vision is the north star here. Think in months and years, not just today. Surface academic or racing context only if it directly connects to something she's building.`,
    personal: `\n\nACTIVE MODE: Personal. This is Brooke's quieter, grounding mode. Lead with her anchors - Bible reading streak, gym sessions, morning routine, the Sydney vision. Keep the tone calm and reflective. No task lists, no Canvas deadlines, no academic pressure. Canvas data has been withheld from your context intentionally. This is about who she is, not what she has to do.`,
  };
  ...
  // Morning routine + evening wind-down + weekly review status
  ...
  const weeklyReviewDone = weeklyReviewLog?.[thisWeekKey]?.completed;
  ...
  const routineLines = [
    ...
    weekIntention ? `This week's intention: "${weekIntention}"` : '',
  ].filter(Boolean).join(' ');
